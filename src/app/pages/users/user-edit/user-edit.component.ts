...
export class UserEditComponent {
  ...
  _location = inject(Location); // para obtener el estado de una ruta (objeto que se envia)
  ...
  setCurrentPlayer(player: any) {
    this.form.patchValue(player); // carga el nombre, ya que compara nombres que coinciden con el form
    player.decks.map((deck: any) => {
      const deckForm = new FormGroup({
        name: new FormControl(deck.name),
        cards: new FormControl(deck.cards),
      });
      (this.form.get('decks') as FormArray).push(deckForm); // agregamos los deck
    });
  }
